...
class PyHubIDE {
    ...
    setupEventListeners() {
        ...
        this.elements.resetBtn.addEventListener('click', () => this.resetEnvironment());
    ...
    async initializePyodide() {
        try {
            ...
            this.showOutput('✅ ¡PyHub IDE listo!\n🐍 Python 3.11 ejecutándose en tu navegador\n\nComienza escribiendo código y presiona Ctrl+Enter para ejecutar.', 'info');
    ...
    async runCode() {
        ...
        try {
            ...
            if (!validation.valid) {
                this.showOutput(`❌ Errores de validación:\n${validation.errors.join('\\n')}`, 'error');
                return;
            }

            if (validation.warnings.length > 0) {
                this.showOutput(`⚠️ Advertencias:\n${validation.warnings.join('\\n')}\\n`, 'warning');
            }
            ...
            if (result.success) {
                ...
                this.showOutput(output || '✅ Código ejecutado correctamente (sin salida)', 'success');
            } else {
    ...
    async runTests() {
        ...
        try {
            ...
            if (!validation.valid) {
                this.showOutput(`❌ No se encontraron tests válidos:\\n${validation.errors.join('\\n')}`, 'error');
                return;
            }
            ...
            if (result.success) {
                const formattedResults = this.testRunner.formatTestResults(result.results);
                this.showOutput(formattedResults, 'success');
                
                // Mostrar estadísticas adicionales
                this.showTestSummary(result.results);
            } else {
                this.showOutput(`❌ Error al ejecutar tests:\\n${result.error}\\n\\n${result.output || ''}`, 'error');
            }

        } catch (error) {
            this.showOutput(`💥 Error inesperado en tests:\\n${error.message}`, 'error');
        } finally {
            this.setRunning(false);
        }
    }
    ...
    async resetEnvironment() {
        ...
        try {
            ...
            if (result.success) {
                this.showOutput('✅ Entorno reiniciado correctamente\\nTodas las variables y módulos han sido limpiados.', 'success');
            } else {
    ...
    async loadExample(filename) {
        try {
            ...
            if (response.ok) {
                ...
                this.showOutput(`📚 Ejemplo cargado: ${exampleName}\\n\\n${this.getExampleDescription(filename)}`, 'info');
            } else {
                this.showOutput(`❌ No se pudo cargar el ejemplo: ${filename}`, 'error');
            }
        } catch (error) {
            this.showOutput(`❌ Error al cargar ejemplo:\\n${error.message}`, 'error');
        }
    }
    ...
    showOutput(text, type = 'normal') {
        ...
        outputDiv.className = 'output-entry fade-in-up mb-3 p-3 rounded-lg border-l-4';
        ...
        switch (type) {
            case 'error':
                ...
                break;
            case 'warning':
                outputDiv.className += ' bg-yellow-900/20 border-yellow-500 text-yellow-300';
                break;
            case 'success':
                outputDiv.className += ' bg-green-900/20 border-green-500 text-green-300';
                break;
            case 'info':
    ...
    showTestSummary(results) {
        ...
        summary.className = 'mt-4 p-4 bg-gray-800 rounded-lg border border-gray-600';
        
        const successRate = results.success_rate || 0;
        const barColor = successRate >= 80 ? 'green' : successRate >= 60 ? 'yellow' : 'red';
        
        summary.innerHTML = `
            <div class="flex items-center justify-between mb-3">
                <h4 class="font-semibold text-white">📊 Resumen de Tests</h4>
                <span class="text-sm text-gray-400">${results.duration || 0}ms</span>
            </div>
            
            <div class="grid grid-cols-3 gap-4 mb-3 text-center">
                <div class="p-2 bg-gray-700 rounded">
                    <div class="text-lg font-bold text-blue-400">${results.total}</div>
                    <div class="text-xs text-gray-400">Total</div>
                </div>
                <div class="p-2 bg-gray-700 rounded">
                    <div class="text-lg font-bold text-green-400">${results.passed}</div>
                    <div class="text-xs text-gray-400">Pasados</div>
                </div>
                <div class="p-2 bg-gray-700 rounded">
                    <div class="text-lg font-bold text-red-400">${results.failed || results.errors || 0}</div>
    ...
    setRunning(running) {
        ...
        if (running) {
            ...
            this.elements.statusDot.className = 'status-dot bg-yellow-500 animate-pulse-slow';
        } else {
            this.updateStatus('Python listo', 'green');
    ...
    enableButtons(enabled) {
        const buttons = [this.elements.runBtn, this.elements.testBtn, this.elements.resetBtn];
        buttons.forEach(btn => {
...
print("\\n✨ ¡Ejecutado correctamente desde el navegador!")`;
    ...
    getExampleDescription(filename) {
        const descriptions = {
            'hello_world.py': 'Conceptos básicos: variables, funciones, bucles y estructuras de datos.',
            'fibonacci.py': 'Algoritmos recursivos e iterativos, optimización y análisis matemático.',
            'tests_example.py': 'Framework de testing, aserciones y buenas prácticas de testing.'
        };
