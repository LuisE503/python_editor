...
function OutputPanel({ output, testResults, onClear }) {
  ...
  const hasGraphics = output && (output.includes('plt.show()') || output.includes('Gráfico'));
  ...
  const formatLine = (line, index) => {
    ...
    if (line.includes('Error') || line.includes('Traceback') || line.includes('✗')) {
    ...
    return (
      <div key={index} className={className}>
        <span className="font-mono text-sm">{icon}{line || '\u00A0'}</span>
      </div>
    );
  };
          ...
          <div className="min-h-full">
            {!hasOutput ? (
              <div className="flex flex-col items-center justify-center h-full py-12 text-gray-500">
                ...
                <p className="text-lg">La salida del código aparecerá aquí...</p>
                <p className="text-sm mt-2 text-gray-600">
                  💡 Tip: Usa <kbd className="bg-gray-800 px-2 py-0.5 rounded text-gray-400">Ctrl+Enter</kbd> para ejecutar
                </p>
              </div>
            ) : (
              <div className="py-2">
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl p-4 text-white">
              <div className="flex items-center justify-between mb-2">
                <span className="text-2xl font-bold">📊 Resultados de Tests</span>
                <span className={`text-3xl font-bold ${
                  testResults.passed === testResults.total ? 'text-green-300' : 'text-red-300'
                }`}>
              ...
              <div className="w-full bg-white bg-opacity-20 rounded-full h-3">
                <div
                  className={`h-3 rounded-full transition-all ${
                    testResults.passed === testResults.total ? 'bg-green-400' : 'bg-red-400'
                  }`}
                  style={{ width: `${(testResults.passed / testResults.total) * 100}%` }}
                ></div>
              </div>
              <p className="text-sm mt-2 text-purple-200">
                {((testResults.passed / testResults.total) * 100).toFixed(1)}% de tests pasados
              </p>
            </div>
            ...
            <div className="space-y-2">
              {testResults.tests.map((test, index) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className={`text-xl ${test.passed ? 'text-green-400' : 'text-red-400'}`}>
                      {test.passed ? '✓' : '✗'}
                    </span>
